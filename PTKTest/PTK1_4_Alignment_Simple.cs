...
namespace PTK
{
    public class PTK1_4_Alignment_Simple : GH_Component
    {
        ...
        {
            pManager.AddTextParameter("AlignmentName", "a", "Alignmentname is optional. One or similiar amount as curve", GH_ParamAccess.item);
            ...
            pManager.AddNumberParameter("Offset Local z", "local y", "Offset length local z", GH_ParamAccess.list);
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            if (Functions.CheckInputValidity(listlengths))
            {
                ...
                {
                    simplAlign.Add(new Align(alignmentname, globalZvector[i], offsetY[i], offsetZ[i]));   
                }
            }
            else
            {
