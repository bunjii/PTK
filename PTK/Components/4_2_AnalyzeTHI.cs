...
namespace PTK
{
    public class PTK_4_2_AnalyzeTHI : GH_Component
    {
        ...
        public Assembly Add_Forces_From_Karamba(Assembly assembly, Karamba.Models.Model model)
        {
                ...
                foreach (var result_point_list in result_element_list)
                {
                    ...
                    foreach (var force_component_list in result_point_list)
                    {
                        ...
                        if (N1_compression_max >= force_component_list[0])
                        {
                            ...
                            tmpForces.Position_Max_Fx_compression = assemble.Elems[element_id].Crv.PointAt(Convert.ToDouble(id_points) / Convert.ToDouble(maximum_num_points));
                        ...
                        if (N1_tension_max <= force_component_list[0])
                        {
                            ...
                            tmpForces.Position_Max_Fx_tension = assemble.Elems[element_id].Crv.PointAt(Convert.ToDouble(id_points) / Convert.ToDouble(maximum_num_points));
                        }
                        if (N2_max <= Math.Abs(force_component_list[1]))
                        {
                            ...
                            tmpForces.Position_Max_Fy_shear = assemble.Elems[element_id].Crv.PointAt(Convert.ToDouble(id_points) / Convert.ToDouble(maximum_num_points));
                        }
                        if (N3_max <= Math.Abs(force_component_list[2]))
                        {
                            ...
                            tmpForces.Position_Max_Fz_shear = assemble.Elems[element_id].Crv.PointAt(Convert.ToDouble(id_points) / Convert.ToDouble(maximum_num_points));
                        }
                        if (M1_max <= Math.Abs(force_component_list[3]))
                        {
                            ...
                            tmpForces.Position_Max_Mx_torsion = assemble.Elems[element_id].Crv.PointAt(Convert.ToDouble(id_points) / Convert.ToDouble(maximum_num_points));
                        }
                        if (M2_max <= Math.Abs(force_component_list[4]))
                        {
                            ...
                            tmpForces.Position_Max_My_bending = assemble.Elems[element_id].Crv.PointAt(Convert.ToDouble(id_points) / Convert.ToDouble(maximum_num_points));
                        }
                        if (M3_max <= Math.Abs(force_component_list[5]))
                        {
                            ...
                            tmpForces.Position_Max_Mz_bending = assemble.Elems[element_id].Crv.PointAt(Convert.ToDouble(id_points) / Convert.ToDouble(maximum_num_points));
                        }
                    ...
                    //フィールドへの直接SETになっている、非推奨
                    assemble.Elems[element_id].Force = tmpForces;

                    assemble.Elems[element_id].Force.Max_Fx_compression = Math.Abs(N1_compression_max);
                    assemble.Elems[element_id].Force.Max_Fx_tension = Math.Abs(N1_tension_max);
                    assemble.Elems[element_id].Force.Max_Fy_shear = Math.Abs(N2_max);
                    assemble.Elems[element_id].Force.Max_Fz_shear = Math.Abs(N3_max);
                    assemble.Elems[element_id].Force.Max_Mx_torsion = Math.Abs(M1_max);
                    assemble.Elems[element_id].Force.Max_My_bending = Math.Abs(M2_max);
                    assemble.Elems[element_id].Force.Max_Mz_bending = Math.Abs(M3_max);
                }
            ...
            {
                nlist1.Add(e2.Force.Position_Max_My_bending);
