...
namespace PTK
{
    ...
    public class ElemTag : Descriptions
    {
        #region fields
        private string tagIs = "N/A";
   
        #endregion

        #region constructors
        public ElemTag(string _tagIs)
        ...
        public bool check(Detail _detail)
        {
             ...
             List<Element> _elems = _detail.Elems;
                ...
                for (int i = 0; i < elemIds.Count; i++)
                {
                    ...
                    if (_elems[elemIndex].Tag.StartsWith(tagIs) || _elems[elemIndex].Tag.EndsWith(tagIs) || _elems[elemIndex].Tag.Equals(tagIs))
                    {
                        valid = true;
                    }
                    else
